...
function WriteQrcBegin {
    ...
    $prefix='    <qresource prefix="/FluentUI">' 
...
function WriteQrcLine {
    ...
    if ($info -is [System.IO.FileInfo]) {
        ...
        if ($filePath.StartsWith('../Controls/')) 
        {
...
function Main {
    ...
    "module FluentUI" | Out-File -Append -Encoding "utf8" $qmldirFileName
    # write qmldir
    Get-ChildItem -Recurse Controls -Exclude designer -Filter *.qml  | ForEach-Object -Process {
        WriteQmlDirLine $_
    }
    # clear qrc
    ...
    WriteQrcBegin
    # write qrc content
    Get-ChildItem -Recurse Controls -Filter *.qml | ForEach-Object -Process {
        WriteQrcLine $_
    }
